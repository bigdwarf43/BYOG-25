...
[CreateAssetMenu(fileName = "StraightLaser", menuName = "Ability/StraightLaser")]
public class StraightLaser : ScriptableObject, IAbility
{
    ...
    public void Deactivate(Entity entity, string move_direction)
    {
        if (move_direction != null)
        {
            ...
            if (moveTile)
            {
                List<Tile> tile_list = entity.currentTile.world_map.current_map.FindAllTilesInDirection(moveTile, move_direction);
